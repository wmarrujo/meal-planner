<script lang="ts">
	import type {ISOTimeString} from "$lib/cache.svelte"
...
<div class="dropdown {customClasses}">
	<div tabindex="0" role="button" class="btn btn-sm w-12 m-1">{value?.slice(0, 5) ?? "__:__"}</div>
	<ul class="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow max-h-40 overflow-y-scroll">
		{#each options as option (option)}
			<li><button class="btn btn-sm" onclick={() => { value = option; onchange(value) }}>{option.slice(0, 5)}</button></li>
		{/each}
	</ul>
</div>
